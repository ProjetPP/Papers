...
The standard form is send into the core. The core's job is routing. It have to send the request in each module to obtain the answers. Sometime, a module can request an answer of another module, for example in the question "what is two times the population of the United States?" which need a geographic data and a mathematical operation. Such a question needs the successive work of two modules.
...
innovations technologiques : open source question answering system, putting together some natural language processing libraries

approche technique choisie : quickly obtain first results, then improve them depending on the remaining time

implementation of neuronal networks to perform machine learning
...
For the final term, we plan to have an enhanced NLP parser (especially for the machine learning-based one), a simple web front-end and an fully operational Wikidata module.
